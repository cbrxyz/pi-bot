...
async def is_launcher(ctx):
    ...
    if any(r in [staffRole, vipRole, lhRole] for r in member.roles):
        return True
...
async def is_admin(ctx):
    ...
    if aRole in member.roles or member.id == 715048392408956950:
        return True
    ...
    async def predicate(ctx):
        ...
        for c in blacklist:
            if channel == discord.utils.get(server.text_channels, name=c):
                raise CommandNotAllowedInChannel(
                    channel, "Command was invoked in a blacklisted channel."
                )
        return True
    ...
    async def predicate(ctx):
        ...
        for c in whitelist:
            if channel == discord.utils.get(server.text_channels, name=c):
                return True
        raise CommandNotAllowedInChannel(
            channel, "Command was invoked in a non-whitelisted channel."
        )
