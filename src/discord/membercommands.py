...
class MemberCommands(commands.Cog):
    ...
    @discord.commands.slash_command(
        guild_ids=[SLASH_COMMAND_GUILDS], description="Looking for help? Try this!"
    )
    ...
    @discord.commands.slash_command(
        guild_ids=[SLASH_COMMAND_GUILDS], description="Toggles your pronoun roles."
    )
    async def pronouns(
        self,
        ctx,
        pronouns: Option(
            str,
            "The pronoun to add/remove from your account.",
            choices=[ROLE_PRONOUN_HE, ROLE_PRONOUN_SHE, ROLE_PRONOUN_THEY],
            required=True,
        ),
    ):
    ...
    async def profile(
        ...
        username: Option(
            str,
            "The username to get information about. Defaults to your nickname/username.",
            required=False,
        ),
    ):
        if username == None:
    ...
    @discord.commands.slash_command(
        guild_ids=[SLASH_COMMAND_GUILDS], description="Toggles the Alumni role."
    )
    ...
    async def division(
        ...
        div: Option(
            str,
            "The division to assign the user with.",
            choices=["Division A", "Division B", "Division C", "Alumni", "None"],
            required=True,
        ),
    ):
        if div == "Division A":
            ...
            await ctx.interaction.response.send_message(
                content="Assigned you the Division A role, and removed all other divison/alumni roles."
            )
        elif div == "Division B":
            await self._assign_div(ctx, "Division B")
            await ctx.interaction.response.send_message(
                content="Assigned you the Division B role, and removed all other divison/alumni roles."
            )
        elif div == "Division C":
            await self._assign_div(ctx, "Division C")
            await ctx.interaction.response.send_message(
                content="Assigned you the Division C role, and removed all other divison/alumni roles."
            )
        elif div == "Alumni":
            await self._assign_div(ctx, "Alumni")
            await ctx.interaction.response.send_message(
    ...
    async def games(self, ctx):
        ...
        games_channel = discord.utils.get(
            ctx.author.guild.text_channels, name=CHANNEL_GAMES
        )
        member = ctx.author
    ...
    async def states(
        ...
        states: Option(
            str,
            "The states to toggle. For example 'Missouri, Iowa, South Dakota'.",
            required=True,
        ),
    ):
    ...
    @discord.commands.slash_command(
        guild_ids=[SLASH_COMMAND_GUILDS], description="Mutes yourself."
    )
    async def selfmute(
        self,
        ctx,
    ...
    async def request(
        ...
        invitational: Option(
            str,
            "The official name of the invitational you would like to add.",
            required=True,
        ),
    ):
    ...
    async def link(
        ...
        destination: Option(
            str,
            "The area of the site to link to.",
            choices=["forums", "exchange", "gallery", "obb", "wiki", "tests"],
            required=True,
        ),
    ):
    ...
    async def random(
        ...
        minimum: Option(
            int,
            "The minimum number to choose from. Defaults to 0.",
            required=False,
            default=0,
        ),
        maximum: Option(
            int,
            "The maximum number to choose from. Defaults to 10.",
            required=False,
            default=10,
        ),
    ):
    ...
    async def coach(self, ctx):
        ...
        await ctx.interaction.response.send_message(
            "If you would like to apply for the `Coach` role, please fill out the form here: <https://forms.gle/UBKpWgqCr9Hjw9sa6>.",
            ephemeral=True,
        )
    ...
    async def info(self, ctx):
        ...
        discovery_splash_url = (
            server.discovery_splash.url if server.discovery_splash != None else "None"
        )
    ...
    async def wikisummary(
        ...
        page: Option(
            str,
            "The name of the page to return a summary about. Correct caps must be used.",
            required=True,
        ),
    ):
    ...
    @discord.commands.slash_command(
        guild_ids=[SLASH_COMMAND_GUILDS], description="Links to a particular wiki page."
    )
    async def wikilink(
        self,
        ctx,
        page: Option(
            str, "The wiki page to link to. Correct caps must be used.", required=True
        ),
    ):
    ...
    async def wikipedia(
        ...
        command: Option(
            ...
            choices=["search", "summary", "link"],
            required=True,
        ),
        request: Option(
            str,
            "The request to execute the command upon. What to search or summarize, etc.",
            required=True,
        ),
    ):
        if command == "search":
    ...
    @discord.commands.slash_command(
        guild_ids=[SLASH_COMMAND_GUILDS], description="Toggles event roles."
    )
    async def events(
        self,
        ctx,
    ...
    @discord.commands.slash_command(
        guild_ids=[SLASH_COMMAND_GUILDS], description="Gets a tag."
    )
    async def tag(
        self, ctx, tag_name: Option(str, "The name of the tag to get.", required=True)
    ):
        member = ctx.author
